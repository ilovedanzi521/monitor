<template>
  <div class="table-contanier">
    ...
    <win-table ref="multipleTable2" style="width: 100%" @cell-dblclick="mclick" :data="userReqVo.userArray" :showSelection="true" @select-change="tableSelectionChange" @select-all="tableSelectionChange"  max-height="600">

      <win-table-column prop="ipAddress" label="IP" width="150"></win-table-column>
      <win-table-column prop="name" label="规则分组名称" width="150"></win-table-column>
      <!--<win-table-column :formatter="formatRoleName" label="状态" width="200"></win-table-column>-->
      <win-table-column prop="cpu" label="告警规则的名称" width="150"></win-table-column>
      <win-table-column prop="memory" label="PromQL表达式" width="150"></win-table-column>
      <win-table-column prop="disk" label="评估等待时间" width="150"></win-table-column>
      <win-table-column prop="disk2" label="告警级别" width="150"></win-table-column>
      <win-table-column prop="disk2" label="摘要" width="150"></win-table-column>
      <win-table-column prop="disk3" label="描述" width="150"></win-table-column>
      <win-table-column prop="operation" align="center" fixed="right" label="操作" width="200">
        <template slot-scope="scope">
                  <span class="operation" @click="handleEditMachine(scope.row)">
                        <i class="icon icon2 el-icon-edit"></i>修改预警规则配置
                  </span>
          <span class="operation" @click="handleDeleteMachine(scope.row,1)">
                            <i class="icon icon1 el-icon-delete"></i>删除预警规则配置
                  </span>
        </template>
      </win-table-column>
    </win-table>
